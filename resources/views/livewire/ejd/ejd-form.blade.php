    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @else
            <form wire:submit="generateForm" class="space-y-8">
                ...
                {{-- Section 3: Job Selection (shows when location selected) --}}
                @if($location)
                    @include('livewire.ejd.partials._job-selection')
                @endif

                {{-- Section 4: Tasks (shows when job(s) selected) --}}
                @if(count($jobTitle) > 0)
                    @include('livewire.ejd.partials._tasks')
                @endif

                {{-- Section 5: Physical Demands (shows when tasks selected) --}}
                @if(count($tasks) > 0)
                ...
                @if(count($tasks) > 0)
                    <div class="flex justify-center pt-6">
                        <button type="submit"
                                class="inline-flex items-center px-8 py-3 border border-transparent text-lg font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
